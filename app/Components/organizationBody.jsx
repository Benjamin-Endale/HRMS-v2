...
const OrganizationMainBody = ({ readPath, modules, session }) => {
  ...
  // read from url
  const params = useParams(); // may be undefined if not rendered under a route providing it
  const pathname = usePathname(); // e.g. /Admin/OrganizationPages/40.../Dashboard
  const organizationId = params?.organizationId || extractOrgIdFromPath(pathname);

  // Debug logs (remove when done)
  console.log('OrganizationMainBody: params=', params, 'organizationId=', organizationId, 'pathname=', pathname);
  ...
  useEffect(() => {
    const fetchOrganizations = async () => {
      try {
        ...
        if (res?.length > 0 && !selectedOrg) {
          // set a friendly display value but keep selectedOrg as object or id if you prefer
          setSelectedOrg(res[0].name);
        }
      } catch (err) {
  ...
  }, [tenantId, token]); // intentionally not adding selectedOrg to avoid re-run loop

  // If the org-switch dropdown is used (different feature)
  const handleOrgSelect = (orgName) => {
    const org = organizations.find((o) => o.name === orgName);
    if (!org) return;
    setSelectedOrg(orgName);
    // navigate to org root (Dashboard)
    router.push(`/Admin/OrganizationPages/${org.id}/Dashboard`);
  };

  // menu items: note these are *relative* to the organization base path
  const menuSections = [
    {
      title: 'CORE',
      items: [
        ...
        { path: 'Attendance', label: 'Attendance', icon: Attendance, moduleKey: 'attendanceTracking' },
        { path: 'LeaveManagment', label: 'Leave Management', icon: LeaveManegment, moduleKey: 'leaveManagement' },
        { path: 'UserAuthentication/All', label: 'User Authentication', icon: UserAuth },
      ],
    },
    {
      title: 'TALENT',
      items: [
        { path: 'RecruitmentPages/Jobposting', label: 'Recruitment', icon: Recruitment, moduleKey: 'recruitment' },
        { path: 'PerformancePages/Overview', label: 'Performance', icon: Performance, moduleKey: 'performanceManagement' },
        { path: 'TrainingPages/OverviewTraining', label: 'Training', icon: Training, moduleKey: 'trainingDevelopment' },
      ],
    },
    {
      title: 'OPERATION',
      items: [
        { path: 'Announcement', label: 'Announcement', icon: Announcement },
        { path: 'SettingPages/General', label: 'Settings', icon: Settings },
      ],
    },
  ];

  // Active detection: prefer pathname-based startsWith check (robust for nested routes)
  const makeFullPath = (orgId, itemPath) =>
    orgId ? `/Admin/OrganizationPages/${orgId}/${itemPath}` : `/Admin/OrganizationPages/${itemPath}`;

  const isActiveByPath = (orgId, itemPath) => {
    if (!pathname) return false;
    const full = makeFullPath(orgId, itemPath).toLowerCase();
    // if current pathname starts with the full path OR the full path startsWith pathname (depending on structure)
    return pathname.toLowerCase().startsWith(full);
  };

  // Old readPath logic kept for backward compatibility, but prefer isActiveByPath
  const isActive = (item) => isActiveByPath(organizationId, item.path) || readPath === item.path.replace('/', '');
            ...
            <section key={section.title} className="space-y-[1.5625rem] w-full pl-[2.75rem] relative">
              ...
              {section.items.map((item) => {
                ...
                const path = organizationId
                  ? makeFullPath(organizationId, item.path)
                  : null; // null when there is no org context

                const active = isActive(item);

                return (
                  <div key={item.path} className="flex items-center">
                    <div className={`${active ? 'flex' : 'hidden'} absolute left-0 navBarhover`} />
                    <div className="navLinkconfig">
                      <Icon readPath={readPath} />
                      <div>
                        {path ? (
                          <Link href={path}>
                            <h4 className={`${active ? 'text-white' : 'text-limegray'}`}>{item.label}</h4>
                          </Link>
                        ) : (
                          // show non-clickable label if orgId missing
                          <span className="text-limegray cursor-not-allowed" title="Select organization first">
                            {item.label}
                          </span>
                        )}
                      </div>
...
export default OrganizationMainBody;

/* ---------------------------
   Helper: attempt to extract org id if useParams was not available
   We expect URL shapes like:
   /Admin/OrganizationPages/<orgId>/...  or /Admin/OrganizationPages/<orgId>
   This helper is a safe fallback and won't replace correct useParams.
----------------------------*/
function extractOrgIdFromPath(path) {
  try {
    if (!path) return null;
    const parts = path.split('/').filter(Boolean); // removes empty strings
    // find "OrganizationPages" segment then take the next part as orgId
    const idx = parts.findIndex((p) => p.toLowerCase() === 'organizationpages');
    if (idx >= 0 && parts.length > idx + 1) {
      const possibleId = parts[idx + 1];
      // quick sanity: UUID-ish (contains dashes and length >= 10)
      if (possibleId && possibleId.includes('-') && possibleId.length >= 8) return possibleId;
      return possibleId; // return anyway; backend will reject if invalid
    }
    return null;
  } catch (e) {
    return null;
