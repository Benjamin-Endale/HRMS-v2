...
const DepartmentAdd = ({org,tenantId,employees,dep}) => {
  ...
  const perPage = 8
  const totalPages = Math.ceil(dep.length / perPage)
  const startIndex = (page - 1) * perPage
  const currentDepartments = dep.slice(startIndex, startIndex + perPage)
      ...
      <div className='space-y-[4.0625rem] font-semibold overflow-hidden'>
          ...
          <div className='flex gap-[1.375rem]'>
            <div className='w-[14.875rem] h-[3.4375rem] bg-lemongreen rounded-[0.625rem]'>
              ...
              <ModalContainerDep open={isOpen}>
                <AddDep tenantId={tenantId} employees={employees} org={org} onClose={() => setIsOpen(false)} />
              </ModalContainerDep>
            </div>
          </div>
        </div>
        ...
        <div className='space-y-[1.4375rem] pb-[7rem]'>
          <div className='between-center'>
            <div>
              <h1 className='text-white'>Department Overview ({dep.length})</h1>
            </div>
            ...
            <AnimatePresence mode='wait' custom={direction}>
              ...
              >
                {currentDepartments.map((dept) => (
                  <div key={dept.departmentName} className='relative organizationCard'>
                    <button className='flex flex-col gap-[3.75rem] cursor-pointer' onClick={()=>router.push(`/Admin/DepartmentView/${dept.departmentId}`)}>
                        <div className='flex flex-col items-start'>
                          <h1 className='textWhite'>{dept.departmentName}</h1>
                          <h4 className='text-limegray'>Head: {dept.departmentHead}</h4>
                        </div>
