...
const page = ({users: initialUsers, token}) => {
  ...
  const { searchTerm, setSearchTerm, filteredItems: filteredUsers } = useSearchFilter(users, [
    "firstName",
    "lastName",
    "employeeCode",
    "fullName"
  ...
  return (
      <div className='font-semibold'>
        <SubNavigation readPath= '/Admin/AssignDepartment/Categorized'/>
              ...
              <tbody>
                {filteredUsers.map((usr)=> (
                  <tr key={usr.email}>
                    ...
                    <td className='pt-[2.25rem]'>
                        <h1 className='textLimegray1'>{usr.fullName}</h1>
                    </td>
                    <td className='pt-[2.25rem] '>
                        <h1 className='textLimegray1'>{usr.email}</h1>
                    </td>
                    <td className='pt-[2.25rem]'>
                        <h1 className='textLimegray1'>{usr.position || '--'}</h1>
                    </td>
                    <td className='pt-[2.25rem]'>
                        <h1 className='textLimegray1'>{usr.roles}</h1>
                    </td>
                    <td className='flex items-center gap-[2.5625rem] pt-[2.25rem]'>
                      <button onClick={() => {
                      setSelectedUser(usr);setisOpen(true);}} type="button" className='cursor-pointer'>
                      ...
                      {/* Modal */}
                        <ModalContainerRevoke open={isOpen}>
                            <Revoke
                            Header='Are you sure?'
                            Parag={`Are you sure you want to revoke ${selectedUser?.fullName || 'this user'}?`}
                            confirmation='Revoke'
